...
export const makeClassExp = (
  fields: VarDecl[],
  methods: Binding[]
): ClassExp => ({ tag: "ClassExp", fields: fields, methods: methods });
...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> =>
  ...
  isNonEmptyList(methods[0])
    ...
    : makeFailure(
        `Invalid format for ClassExp ${format(fields)}, ${format(methods)}`
      );
...
const unparseClassExp = (ce: ClassExp): string =>
  `(class (${map((f: VarDecl) => f.var, ce.fields).join(" ")}) (${map(
    (m: Binding) => `(${m.var.var} ${unparseL3(m.val)})`,
    ce.methods
  ).join(" ")}))`;
